...
- **`feat/pan-grouping`, PUSHED. This is where the work is.** Its last *code*
  commit is `a883df6`, with docs-only commits on top carrying this handoff — a
  stamp cannot name the commit that writes it, and a commit count would itself
  rot. The check is:
  ...
  **Empty means this prompt is current.** Any output means the branch moved after
  it was written.
