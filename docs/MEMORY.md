...
**`main @ 333a3f1`**, no branch in flight, this refresh riding on top as a
docs-only commit. A stamp cannot name the commit that writes it, so the
check is not a commit count — counts rot — but this:

```
git log --oneline 333a3f1..main -- src tests frontend docs ":(exclude)docs/MEMORY.md" ":(exclude)docs/NEXT_SESSION_PROMPT.md"
```
...
- **`main` @ `333a3f1`, pushed, in sync with `origin/main`.** The milestone
